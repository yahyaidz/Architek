  ...
  return (
    ...
    >
      <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 hover:border-pink-500/50 transition-all duration-300 group hover:scale-105 h-full flex flex-col min-h-[400px] lg:min-h-[450px]">
        ...
        <h3 className={`text-xl md:text-2xl font-bold text-white mb-3 md:mb-4 group-hover:text-pink-400 transition-colors leading-tight ${isRTL ? 'font-arabic text-right' : 'text-left'}`}>
          {title}
        </h3>

        <p className={`text-gray-300 mb-6 leading-relaxed text-sm md:text-base flex-grow ${isRTL ? 'font-arabic text-right' : 'text-left'}`}>
          {description}
        </p>
