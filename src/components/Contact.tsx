  ...
  return (
    <section id="contact" className="py-16 md:py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-20">
        <div
          ref={elementRef}
          className={`transform transition-all duration-1000 ${isRTL ? 'rtl' : 'ltr'} ${
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            ...
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 hover:border-pink-500/50 transition-all">
              <h3 className={`text-pink-400 font-bold mb-4 text-base md:text-lg ${isRTL ? 'font-arabic' : ''}`}>{t.lyonOffice}</h3>
              <div className="text-xl md:text-2xl font-bold text-white mb-4">ARCHITEK</div>
              <div className={`text-gray-300 mb-2 text-sm md:text-base ${isRTL ? 'font-arabic' : ''}`}>57 Place de la République</div>
              <div className={`text-gray-300 mb-4 text-sm md:text-base ${isRTL ? 'font-arabic' : ''}`}>69002 LYON</div>
            ...
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 hover:border-cyan-500/50 transition-all">
              <h3 className={`text-cyan-400 font-bold mb-4 text-base md:text-lg ${isRTL ? 'font-arabic' : ''}`}>{t.parisOffice}</h3>
              <div className="text-xl md:text-2xl font-bold text-white mb-4">ARCHITEK</div>
              <div className={`text-gray-300 mb-2 text-sm md:text-base ${isRTL ? 'font-arabic' : ''}`}>58 Rue de Monceau</div>
              <div className={`text-gray-300 mb-4 text-sm md:text-base ${isRTL ? 'font-arabic' : ''}`}>75008 PARIS</div>
              <div className="flex items-center gap-2 text-white text-sm">
                <Phone size={16} />
            ...
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 hover:border-purple-500/50 transition-all">
              <h3 className={`text-purple-400 font-bold mb-4 text-base md:text-lg ${isRTL ? 'font-arabic' : ''}`}>{t.ourSolutions}</h3>
              <div className={`space-y-2 text-gray-300 text-sm ${isRTL ? 'font-arabic' : ''}`}>
                <div className={isRTL ? 'text-right' : ''}>{t.websiteCreation}</div>
                <div className={isRTL ? 'text-right' : ''}>{t.appDevelopment}</div>
                <div className={isRTL ? 'text-right' : ''}>{t.saasDevelopment}</div>
                <div className={isRTL ? 'text-right' : ''}>{t.aiIntegration}</div>
                <div className={isRTL ? 'text-right' : ''}>{t.aiConsulting}</div>
              </div>
            </div>

            {/* Follow Us */}
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 hover:border-pink-500/50 transition-all">
              <h3 className={`text-pink-400 font-bold mb-4 text-base md:text-lg ${isRTL ? 'font-arabic' : ''}`}>{t.followUs}</h3>
              ...
              <div className={`text-gray-300 text-sm mb-4 text-center ${isRTL ? 'sm:text-right font-arabic' : 'sm:text-left'}`}>{t.dontMiss}</div>
              <button 
                onClick={onGetQuote}
                className={`px-4 py-2 md:px-6 bg-gradient-to-r from-emerald-500 via-cyan-500 to-purple-600 hover:from-emerald-600 hover:via-cyan-600 hover:to-purple-700 text-white rounded-lg font-medium w-full transition-all text-sm touch-manipulation mb-3 ${isRTL ? 'font-arabic' : ''}`}
              >
                {t.getQuote}
              </button>
              <div className="text-center">
                <p className={`text-emerald-400 text-xs font-medium ${isRTL ? 'font-arabic' : ''}`}>
